...
export default class KanbanConfigManager extends LightningElement {
    ...
    async refresh() {
        ...
        try {
            ...
            this.configs = (data || []).map((c, idx) => ({ ...c, key: c.Id || `row-${idx}` }));
        } catch (e) {
    ...
    async handleDelete(evt) {
        ...
        if (!row.Id) {
            ...
            const next = [...list];
            next.splice(idx, 1);
            if (isStatus) this.statuses = next; else this.configs = next;
            return;
        }
        this.isLoading = true;
        try {
            ...
            if (isStatus) await this.refreshStatuses(); else await this.refresh();
        } catch (e) {
    ...
    async refreshStatuses() {
        ...
        try {
            ...
            this.statuses = (data || []).map((s, idx) => ({ ...s, key: s.Id || `strow-${idx}` }));
        } catch (e) {
